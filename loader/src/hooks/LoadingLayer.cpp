...
#include <Sapfire/modify/CCLayer.hpp>
...
struct CustomLoadingLayer : Modify<CustomLoadingLayer, LoadingLayer> {
    struct Fields {
        ...
        int m_sapfireLoadStep = 0;
        int m_totalMods = 0;
    };

    static void onModify(auto& self) {
        if (!self.setHookPriority("LoadingLayer::init", sapfire::node_ids::SAPFIRE_ID_PRIORITY)) {
            log::warn("Failed to set LoadingLayer::init hook priority, node IDs may not work properly");
        }
        SAPFIRE_FORWARD_COMPAT_DISABLE_HOOKS_INNER("Switching to fallback custom loading layer")
    }
    ...
    void updateLoadedModsLabel() {
        ...
        auto str = fmt::format("Sapfire: Loaded {}/{} mods", count, m_fields->m_totalMods);
    ...
    bool init(bool fromReload) {
        ...
        m_fields->m_smallLabel->setID("sapfire-small-label");
        ...
        m_fields->m_smallLabel2->setID("sapfire-small-label-2");
        ...
        std::visit(makeVisitor {
            ...
            [&](updater::UpdateFailed const& error) {
                ...
                console::messageBox(
                    ...
                    "https://github.com/sapfire-sdk/sapfire/releases/latest.\n"
    ...
    int getCurrentStep() {
        return m_fields->m_sapfireLoadStep + m_loadStep + getLoadedMods();
    }
    ...
    // hook
    void loadAssets() {
        switch (m_fields->m_sapfireLoadStep) {
        case 0:
...
struct FallbackCustomLoadingLayer : Modify<FallbackCustomLoadingLayer, CCLayer> {
    static void onModify(auto& self) {
        SAPFIRE_FORWARD_COMPAT_ENABLE_HOOKS_INNER("")
        else if (!self.setHookPriority("CCLayer::init", sapfire::node_ids::SAPFIRE_ID_PRIORITY)) {
    ...
    bool init() {
        ...
        auto label = CCLabelBMFont::create(
            "Loading Sapfire without UI, see console for details.",
            "goldFont.fnt"
        );
        ...
        // label->setID("sapfire-small-label");
